...
export async function buildSchema(
  ...
  reporter.log(`building schema start ${process.pid} - ${context.workerNumber}`)

  // we need to "hydrate" plugins in worker so api-runner-node knows about them
  store.dispatch({
    type: `SET_SITE_FLATTENED_PLUGINS`,
    payload: context.plugins,
  })
  ...
  reporter.log(`building schema end ${process.pid} - ${context.workerNumber}`)

  reporter.log(`run query start ${process.pid} - ${context.workerNumber}`)

  const graphql = createGraphQLRunner(store, reporter)

  // this is hardcoded query against schema from gatsby-starter-blog, just to test
  ...
  reporter.log(`run query end ${process.pid} - ${context.workerNumber}`)

  require(`fs-extra`).outputFile(
    `q-worker-${context.workerNumber}.json`,
